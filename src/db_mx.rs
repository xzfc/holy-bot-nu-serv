...
fn update_from_file_inner(conn: &mut Connection, path: &str) -> Result<(), MyError> {
...
fn update(conn: &mut Connection, val: &serde_json::Value) -> Result<(), MyError> {
    macro_rules! try_or {
        ($fail:expr, $e:expr) => {
            match $e {
                Some(x) => x,
...
fn update_item(conn: &mut Connection, time: i64, mxid: &str) -> Result<(), MyError> {
